...
import uuid
import nextcord
from nextcord import Embed, SelectOption, ButtonStyle, Interaction, SlashOption, Attachment
...
class CategorySelect(Select):
    ...
    async def on_item(self, interaction: nextcord.Interaction):
        ...
        # add dropdown to select another directly
        sel2 = Select(
...
class RootView(View):
    def __init__(self):
        ...
        async def refresh_cb(btn, inter: nextcord.Interaction):
...
@bot.slash_command(
    name="addmission",
    description="Voeg nieuwe mission toe",
    guild_ids=[GUILD_ID]
)
async def addmission(
    ...
    await interaction.followup.send(
        f"✅ Mission **{title}** toegevoegd in **{category}** en gepusht!",
        ephemeral=True
    )
