...
def _parent_and_leaf(rel: str) -> Tuple[str, str]:
    rel = rel.strip().strip("/")
    if "/" in rel:
        p, leaf = rel.rsplit("/", 1)
        return p, leaf
...
def _resolve_key_by_listing(category: str, item_rel_base: str) -> Optional[Tuple[str, str]]:
    """
    Fallback: lijst de oudermap en match case-insensitief op naam (met .json/.txt).
    Return (key, ext) of None.
    """
    parent_rel, leaf = _parent_and_leaf(item_rel_base)
    parent_abs = f"{ROOT_PREFIX}/{category}".replace("//", "/")
    if parent_rel:
        parent_abs = f"{parent_abs}/{parent_rel}".replace("//", "/")
    try:
        _dirs, files = list_dir(parent_abs)  # files: [(name, size)]
    except Exception:
        return None
    for name, _ in files:
        nlow = name.lower()
        if nlow.endswith(".json") or nlow.endswith(".txt"):
            base = _strip_ext(name).lower()
            if base == leaf.lower():
                key = f"{parent_abs}/{name}".replace("//", "/")
                ext = ".json" if nlow.endswith(".json") else ".txt"
                return key, ext
    return None

def _find_existing_item_key(category: str, item_rel: str) -> Optional[Tuple[str, str]]:
    """Probeert directe read (.json → .txt). Valt daarna terug op listing (case-insensitief)."""
    ...
    for key in candidates:
        try:
            ...
            continue
    # Fallback door parent te listeren (pikt ook hoofdletter-varianten op)
    return _resolve_key_by_listing(category, _strip_ext(item_rel))
...
def create_dossier_file(category: str, item_rel_input: str, content: str, prefer_txt_default: bool = True) -> None:
    ...
    try:
        ...
        if item_rel.lower().endswith(".json"):
            ...
            save_text(f"{ROOT_PREFIX}/{category}/{item_rel}".replace("//", "/"),
...
class UploadMenuView(View):
    ...
    @nextcord.ui.button(label="📤 Upload File", style=ButtonStyle.primary, custom_id="upload_btn_v3")
    ...
    @nextcord.ui.button(label="🗑️ Remove File", style=ButtonStyle.danger, custom_id="remove_btn_v3")
...
class UploadFileView(View):
    def __init__(self):
        ...
        self.cat_select = Select(
            ...
            min_values=1, max_values=1, custom_id="upload_cat_select_v3"
        )
    ...
    async def select_category(self, interaction: Interaction):
        ...
        self.role_select = Select(
            ...
            min_values=1, max_values=1, custom_id="upload_role_select_v3"
        )
        self.role_select.callback = self.select_role
        self.add_item(self.role_select)

        submit = Button(label="Step 3: Enter details", style=ButtonStyle.primary, custom_id="upload_open_modal_v3")
...
class UploadDetailsModal(Modal):
    def __init__(self, parent_view: UploadFileView):
        ...
        # <= 45 chars per Discord rules
        self.item = TextInput(label="File name (subfolders ok)")
        self.content = TextInput(label="Content (JSON or Text)", style=TextInputStyle.paragraph)
...
class RemoveFileView(View):
    def __init__(self):
        ...
        self.cat_select = Select(
            placeholder="Step 1: Select category…",
            options=[SelectOption(label=c.replace("_"," ").title(), value=c) for c in cats],
            min_values=1, max_values=1, custom_id="remove_cat_select_v3"
        )
        self.cat_select.callback = self.select_category
        self.add_item(self.cat_select)
    ...
    async def select_category(self, interaction: Interaction):
        ...
        self.item_select = Select(
            ...
            min_values=1, max_values=1, custom_id="remove_item_select_v3"
        )
...
class CategorySelect(View):
    ...
    def __init__(self):
        ...
        self.sel = Select(
            ...
            options=[SelectOption(label=c.replace("_"," ").title(), value=c) for c in cats],
            min_values=1, max_values=1, custom_id="cat_select_v3"
        )
    ...
    def build_item_list_view(self, category: str):
        ...
        if items:
            sel_item = Select(
                ...
                options=[SelectOption(label=i, value=i) for i in items[:25]],
                min_values=1, max_values=1, custom_id="cat_item_select_v3"
            )
...
async def grantfileclearance_cmd(inter: Interaction):
    ...
    if not (inter.user.id == inter.guild.owner_id or inter.user.guild_permissions.administrator or (user_roles & ALLOWED_ASSIGN_ROLES)):
        return await inter.response.send_message("⛔ ARCHIVIST ONLY.", ephemeral=True)
    await inter.response.send_message("Use the explorer for now; per-file grant UI komt hierna.", ephemeral=True)
...
async def revokefileclearance_cmd(inter: Interaction):
    ...
    if not (inter.user.id == inter.guild.owner_id or inter.user.guild_permissions.administrator or (user_roles & ALLOWED_ASSIGN_ROLES)):
        return await inter.response.send_message("⛔ ARCHIVIST ONLY.", ephemeral=True)
    await inter.response.send_message("Use the explorer for now; revoke UI komt hierna.", ephemeral=True)
