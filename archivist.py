...
async def refresh_menus(
    ...
    if isinstance(stored_anchor, dict):
        anchor_channel_id = _coerce_channel_id(stored_anchor.get("channel_id"))
    ...
    old_anchor_channel = None
    if old_message_id is not None and anchor_channel_id is not None:
        if callable(get_channel_or_thread):
            old_anchor_channel = get_channel_or_thread(anchor_channel_id)
        else:
            old_anchor_channel = guild.get_channel(anchor_channel_id)
        if not hasattr(old_anchor_channel, "fetch_message"):
            old_anchor_channel = None

    if old_message_id is not None:
        try:
            fetch_channel = old_anchor_channel or menu_ch
            old_message = await fetch_channel.fetch_message(old_message_id)
        except Exception:
            old_message = None
