...
# Verberg systeemdingen overal (menu, search, etc.)
...
def grant_file_clearance(category: str, item_rel_base: str, role_id: int) -> None:
    ...
    if role_id not in cf[category][item_rel_base]:
        cf[category][item_rel_base].append(int(role_id))
    save_clearance(cf)
...
def revoke_file_clearance(category: str, item_rel_base: str, role_id: int) -> None:
    ...
    if category in cf and item_rel_base in cf[category]:
        cf[category][item_rel_base] = [int(r) for r in cf[category][item_rel_base] if int(r) != int(role_id)]
...
def list_categories() -> list[str]:
    ...
    for d in dirs:
        if not d.endswith("/"): 
            continue
        name = d[:-1]
        if name.startswith("_") or name in SYSTEM_DIRS:
            continue
        cats.append(name)
    if not cats:
...
def update_dossier_raw(category: str, item_rel_base: str, new_content: str) -> str:
    ...
    key, ext = found
    try:
        save_version(key)  # één versie vooraf bewaren
    except Exception:
        pass
    if ext == ".json":
        try:
...
def patch_dossier_json_field(category: str, item_rel_base: str, field_path: str, value_text: str) -> str:
    """
    Patch één JSON-veld (dot path) in een dossierbestand. Werkt ook als het bestand .txt is met JSON-inhoud.
    """
    found = _find_existing_item_key(category, item_rel_base)
    if not found:
        raise FileNotFoundError
...
# ===== Permission fallback based on numeric clearance in JSON =====
