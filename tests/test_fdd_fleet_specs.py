...
def test_get_fdd_ships_returns_dynamic_entries(monkeypatch):
    payload = {"ships": [_ship_payload(), _ship_payload(slug="beta", name="Beta")]}

    def fake_read_json(key):
        assert key == fdd_fleet_specs._SHIP_MANIFEST_KEY
        return payload

    monkeypatch.setattr(fdd_fleet_specs, "read_json", fake_read_json)

    ships = fdd_fleet_specs.get_fdd_ships()
    assert len(ships) == 2
    first = ships[0]
...
def test_get_fdd_ships_handles_missing_file(monkeypatch):
...
def test_get_ship_by_slug_normalizes_input(monkeypatch):
    ...
    ship = fdd_fleet_specs.get_ship_by_slug("  Gamma  ")
    assert ship is not None
    assert ship.slug == "gamma"
    assert ship.name == "Gamma Vessel"


def test_save_fdd_ship_spec_updates_existing_entry(monkeypatch):
    ...
    monkeypatch.setattr(fdd_fleet_specs, "read_json", fake_read_json)
    monkeypatch.setattr(fdd_fleet_specs, "save_json", fake_save_json)

    entry = {"slug": "Delta", "name": "Delta", "crew": "18"}
    result = fdd_fleet_specs.save_fdd_ship_spec(entry)

    assert result["slug"] == "delta"
    assert saved["key"] == fdd_fleet_specs._SHIP_MANIFEST_KEY
    assert saved["data"]["ships"][0]["crew"] == "18"


def test_save_fdd_ship_spec_handles_missing_manifest(monkeypatch):
    saved = {}
    ...
    fdd_fleet_specs.save_fdd_ship_spec(entry)
