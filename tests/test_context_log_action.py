...
def test_log_action_broadcasts_to_admin_log_channel(monkeypatch):
    ...
    assert payload["embed"].fields[3].value == "Test admin message"
...
def test_log_action_renders_security_breach_embed(monkeypatch):
    ...
    assert embed.fields[1].value == "Unknown"
    assert embed.fields[2].value == "BLOCKED"
    assert embed.fields[4].value == "personnel/The_Director.txt"


def test_log_action_renders_access_granted_embed_with_clearance_and_case(monkeypatch):
    bot = DummyBot()
    channel = DummyChannel()
    bot._channels[555] = channel
    ...
    asyncio.run(
        context.log_action(
            "@TheDirector accessed intelligence/Sea_of_Thieves_universe.txt with clearance Level 5. Status SUCCESS. Case ID FDD-SC-275."
        )
    )

    embed = channel.messages[0]["embed"]
    assert embed.title == "ACCESS GRANTED"
    assert embed.fields[1].value == "Level 5"
    assert embed.fields[4].value == "intelligence/Sea_of_Thieves_universe.txt"
    assert embed.fields[6].value == "FDD-SC-275"
